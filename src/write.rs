...
impl<W: Write + io::Seek> GenericZipWriter<W> {
    fn switch_to(&mut self, compression: CompressionMethod) -> ZipResult<()> {
        ...
        *self = match compression {
            ...
            CompressionMethod::Deflated => GenericZipWriter::Deflater(
                bare.deflate_encode(flate2::Compression::Default),
            ),
            #[cfg(feature = "bzip2")]
            CompressionMethod::Bzip2 => GenericZipWriter::Bzip2(
                BzEncoder::new(bare, bzip2::Compression::Default),
            ),
...
fn write_local_file_header<T: Write>(writer: &mut T, file: &ZipFileData) -> ZipResult<()> {
    ...
    // file name length
    try!(writer.write_u16::<LittleEndian>(
        file.file_name.as_bytes().len() as u16,
    ));
    // extra field length
...
fn update_local_file_header<T: Write + io::Seek>(
    ...
    try!(writer.seek(
        io::SeekFrom::Start(file.header_start + CRC32_OFFSET),
    ));
    try!(writer.write_u32::<LittleEndian>(file.crc32));
    try!(writer.write_u32::<LittleEndian>(
        file.compressed_size as u32,
...
fn write_central_directory_header<T: Write>(writer: &mut T, file: &ZipFileData) -> ZipResult<()> {
    ...
    try!(writer.write_u32::<LittleEndian>(
        spec::CENTRAL_DIRECTORY_HEADER::signature(),
    ));
    // version made by
    let version_made_by = (file.system as u16) << 8 | (file.version_made_by as u16);
    try!(writer.write_u16::<LittleEndian>(version_made_by));
