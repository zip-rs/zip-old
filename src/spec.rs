...
impl CentralDirectoryEnd {
    pub fn parse<T: Read>(reader: &mut T) -> ZipResult<CentralDirectoryEnd> {
        ...
        println!("disk_number {:x}", disk_number);
        ...
        println!("central_directory_size {:x}", central_directory_size);
    ...
    pub fn find_and_parse<T: Read + io::Seek>(reader: &mut T) -> ZipResult<CentralDirectoryEnd> {
        ...
        let search_upper_bound =
            ::std::cmp::max(0, file_length - header_size - ::std::u16::MAX as i64);
    ...
    pub fn write<T: Write>(&self, writer: &mut T) -> ZipResult<()> {
        ...
        try!(writer.write_u16::<LittleEndian>(
            self.zip_file_comment.len() as u16,
        ));
