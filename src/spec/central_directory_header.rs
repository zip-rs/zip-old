...
impl CENTRAL_DIRECTORY_HEADER {
        ...
        T: Read + Seek,
    {
        ...
        let mut header = CENTRAL_DIRECTORY_HEADER {
            ...
            disk_number_start: reader.read_u16::<LittleEndian>()? as u64,
            internal_file_attrs: reader.read_u16::<LittleEndian>()?,
            external_file_attrs: reader.read_u32::<LittleEndian>()?,
            local_header_offset: reader.read_u32::<LittleEndian>()? as u64,
            file_name: Vec::new(),
        ...
        while rest_of_bytes > 0 {
            ...
            if 0x0001 == extra_field.header_id {
                ...
                if 0xFFFFFFFF == header.local_header_offset {
                    header.local_header_offset = reader.read_u64::<LittleEndian>()?;
                }
                if 0xFFFFFFFF == header.disk_number_start {
                    header.disk_number_start = reader.read_u64::<LittleEndian>()?;
                }
            } else {
                reader.seek(
        ...
        if rest_of_bytes < 0 {
            return Err(ZipError::InvalidArchive(
                "The [Central directory header] contains wrong [extensible data fields]",
            ));
        }
