//! ZIP [`Archive`]s store files in a cross-platform format to be saved and
//! shared.
//!
...
//!
//! > Note: Compression algorithms are able to be configured and optimised for
//!         your files to use even less space
//!
//! ZIP supports many compression formats, and this library is not compatible
//! with all of them yet. Luckily, most ZIP files share the same few formats.
//!
// TODO: List pros and cons of available formats
//!
//! ## Encryption
//!
//! It is also possible to encrypt the contents of a ZIP file. Using this
//! feature, you can use a passphrase to keep private information safe.
//!
// TODO: Add link to explanation
//! > **Be careful** when using this feature, especially to provide privacy to
//!   your users - ZIP encryption only protects a portion of the archive.
//!
// FIXME: Move these details to the implementation?
//! ### Traditional PKWARE Encryption
//!
//! Sometimes known as "ZipCrypto", this has been [found to be vulnerable][Known
//! Plaintext Attack]. Unfortunately, it is the most widely supported spec
//! anyway, and you'll find many archives using it.
// TODO: Explain this library's encryption features - we support decryption!
//!
//! ### AE-x
//!
//! Taking advantage of the well-known AES (that's the *Advanced* Encryption
//! Standard to you) cipher, this method can reliably secure files so long as
//! your recipient can read it. For now it has very spotty support, and is
//! notably missing from Windows Explorer.
//!
//! ## Extensions
//!
//! Files is a ZIP archive each have a header containing a list of extensible
//! fields which contain third party metadata. They don't have a defined format,
//! meaning this library cannot parse them and leaves the task to the user.
//!
...
//! [util.zip]: https://docs.oracle.com/javase/8/docs/api/index.html?java/util/zip/package-summary.html
...
impl<D, M> Archive<D, M> {
    ...
    /// The [`io`] traits are implemented for shared references to std types,
    /// which means you may read from the [`File`]s yielded by this iterator.
    ///
    /// Be careful with these references though - each one refers to the same
    /// object, and share its position. You can only read from the latest
    /// [`File`] yielded.
    ///
    /// ```
    /// // TODO: Example
...
/// An iterator over files within an [`Archive`]
///
/// # Warnings
///
/// See [`Archive::iter`]
