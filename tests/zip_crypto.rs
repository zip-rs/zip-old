...
fn encrypting_file() {
    ...
    archive
        .start_file(
            "name",
            FileOptions::default().with_deprecated_encryption(b"password"),
        )
        .unwrap();
    ...
    archive
        ...
        .unwrap();
    archive.write_all(&[0xde, 0xad, 0x05, 0x00]).unwrap();
    archive.write_all(b"extra").unwrap();
    archive.end_extra_data().unwrap();
    archive.write_all(b"test").unwrap();

    archive.finish().unwrap();
    drop(archive);

    let mut archive = ZipArchive::new(std::io::Cursor::new(&mut buf)).unwrap();
    ...
    let mut file = archive.by_index_decrypt(1, b"password").unwrap().unwrap();
    let mut buf = Vec::new();
    file.read_to_end(&mut buf).unwrap();
    assert_eq!(buf, b"test");
    assert_eq!(&file.extra_data()[4..], b"extra");
}
#[test]
fn encrypted_file() {
    ...
    let mut archive = ZipArchive::new(zip_file_bytes).unwrap();
